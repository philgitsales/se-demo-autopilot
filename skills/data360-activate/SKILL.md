...
### Marketing Cloud Activation Specifics (PROVEN 2026-06-27)

- **MC activation targets MUST be created via UI** — Data Cloud Setup → Marketing → MC Engagement. API-created targets are missing internal configuration (DataConConfigurationId, AudienceDmoNames) and fail silently.
- **MC Connect (et4ae5) is NOT the same thing** — that syncs CRM records to MC. The Data Cloud MC connector is separate.
- **The activation payload requires `activationTargetSubjectConfig`** — `{"developerName": "ssot__Individual__dlm"}` for Individual-based segments.
- **`attributesConfig` is mandatory** — at minimum include the primary key attribute (ssot__Id__c).
- **Optional `contactPointsConfig`** — adds email/phone to the published Data Extension.
- **Discovery method**: Create one activation via UI, then GET `/ssot/activations/{name}` to see the exact payload structure.

See `data-cloud-setup/blockers-and-next-steps.md` for the complete working payload and key IDs.
